...
class TestEnvironment extends PHPUnit_Framework_TestCase
{
    ...
    public function testWhichEnvironmentDefault()
    {
        ...
        $env = new xPress\Config\Environment($locations);
    ...
    public function testWhichEnvironmentWithRegEx()
    {
        ...
        $this->assertEquals('local', $env->which('server001.larapress.loc'));
        $this->assertEquals('local', $env->which('abcserver3578sdd67.larapress.loc'));
        ...
        $locations = [
            'local' => ['host-one', '*.larapress.net', 'eu-2_3-xyz.aws.com']
        ];

        $env = new xPress\Config\Environment($locations);

        $this->assertEquals('local', $env->which('host-one'));
        $this->assertEquals('local', $env->which('host-one.larapress.net'));
